...
void g_class_system_reset() {
  g_auto_container_free("GClassSystem");
  name_to_class = NULL;
}
GObjectClass *g_register_class(const gstr classname, GObjectClass *baseclazz,
  ...
  if (clazz == NULL) {
    ...
    g_return_val_if_fail(clazz->g_class_name, NULL, g_free(clazz));
    clazz->instance_size = instance_size;
    clazz->init = init;
    clazz->finalize = finalize;
    if (class_init != NULL)
      class_init(clazz);
    g_return_val_if_fail(g_map_set(name_to_class, clazz->g_class_name, clazz),
                         NULL, g_free(clazz->g_class_name), g_free(clazz));
  }
  return clazz;
}

...
#define GET_MODE(mode) (mode == FILE_MODE_READ_ONLY ? "rb" : (mode == FILE_MODE_READ_WRITE ? "rb+" : "wb+"))
...
void g_file_stream_reopen(gpointer self)
{
	...
	s->file_handle = (ghandle)freopen(s->file_name, s->file_mode == FILE_MODE_READ_ONLY ? "r" : "rw", s->file_handle);
	if (g_file_stream_is_open(s))
		open_file_count ++;
	//g_log_debug("reopen file %s : %x (count = %d)\n", s->file_name, GPOINTER_TO_INT(s->file_handle), open_file_count);
}
gint g_file_stream_open(gpointer self, gstring name, GFileMode mode)
{
	...
	if(s->file_handle == NULL) {
		g_log_error("open file error: %s %s\n", s->file_name, GET_MODE(s->file_mode));
		return -1;
	}
	if (g_file_stream_is_open(s))
		open_file_count ++;
...
void g_file_stream_close(gpointer self)
{
	if (g_file_stream_is_open(self))
	{
		...
		//g_log_debug("close file %s : %x (count = %d)\n", GFILESTREAM(self)->file_name, GFILESTREAM(self)->file_handle, open_file_count);
